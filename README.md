...
- **Scan cache** — Split per volume so a home scan does not load NAS hashes; Settings shows real on-disk size
- **Precise compare** — Results are saved and reused on the next scan; full-file xxHash; can be stopped; UI stays responsive on tens of thousands of groups

### 1.0.7

- **Search + Clean Up** — Cleanup, Will free, and the preview only include marked files that match the current search (other marks stay until you clear the search)

### 1.0.6

- **Updates** — Optional daily check (and **Check for Updates…**) against GitHub Releases; never downloads or installs
- **Will free** — Shows marked copies in the current filter (same scope as Clean Up)
- **Search** — Typing stays responsive on large result lists
...
- **Scan cache** — Merge per scan root so NAS (`/Volumes/…`) hashes are not wiped by a later home scan; progressive cache flush during long network hashes
- **NAS progress** — Shows `hits · new · conc` so cache reuse is visible when only new files need hashing
